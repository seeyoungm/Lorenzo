...
**스코어러 드리프트가 다시 확인됨**: 스코어러를 재학습할 때마다 도메인별로 이전 최적 아키텍처가 top-k 밖으로 밀려날 수 있음(Tabular 1.0→1.1 사이에도 관찰됐던 현상). 개선이 보장되지 않는다는 뜻이며, 릴리스 파이프라인이 실측으로 검증해 하락을 자동으로 걸러내는 게 핵심 안전장치임을 재확인.

## 실험 기록 — v0.8 (CIFAR-10 도메인 추가)

v0.7에서 보류했던 CIFAR-10(컬러 이미지)을 재도전. 배포처(`cs.toronto.edu`)가 느린 원인은 네트워크 차단이 아니라 **서버의 커넥션당 대역폭 제한**으로 확인(단일 연결 500B/s~15KB/s, 병렬 range 요청은 커넥션 수만큼 거의 선형으로 총 처리량 증가 — `aria2c -x8 -s8`로 8개 동시 연결 시 170MB를 ~30분에 완주). 데이터를 확보한 뒤 `real_image.py`/`release.py`에 `cifar10`을 다시 추가하고 코퍼스(120 프로파일, CIFAR 포함)를 재빌드·재학습(v0.8, 입력 차원은 v0.7과 동일한 42차원 — 색상 채널 수는 프로파일의 `log_input_size`에 이미 반영됨).

**릴리스**: 신규 도메인이라 비교 대상 없이 top-8 검증 결과를 그대로 채택 — **Lorenzo CIFAR 1.0, test 0.799**(4× Conv2D(128, k5, tanh) + residual, adam, `--num-samples 60000 --top-k 8 --epochs 40`). CIFAR의 residual 후보(4블록, 128필터, 5×5커널)는 후보당 최대 ~22분 — MNIST 풀데이터 residual 후보(50분+)보다는 싸지만 여전히 비쌈.

코퍼스 재빌드 전 GPU vs CPU를 실측 벤치마크한 결과도 참고할 만함: tabular/image/timeseries는 CPU가 1.3~4배 빠르지만 text의 재귀 인코더(특히 bilstm/bigru, 시퀀스 길이 200)는 GPU가 2~3배 빠름 — 도메인 균등 분포 가정 시 가중 총합은 GPU가 ~1.9배 빠름(text가 전체 시간을 지배). "GPU가 항상 유리하다"는 가정은 검증 없이 하지 말 것.

## 실험 기록 — v0.9 (텍스트 도메인 두 버그 분리·수정)

오래 남아있던 "Reuters 예측이 0에 붙는다" + "텍스트 후보가 종종 학습 중 붕괴한다"를 파보니 **별개의 두 원인**이었다.

1. **`0.0001` 예측 (Reuters) — 완전 해결.** 스코어러 버그가 아니라 **코퍼스 커버리지 부족**이었다: 텍스트 프로파일이 output_dim 3~8만 담고 있어서 실제 46클래스 Reuters가 학습 분포 밖 → 스코어러가 0으로 외삽. `text_data.py`의 `REUTERS_CLASS_RANGE`를 (3,9)→(3,47)로 넓히고 텍스트 도메인만 증분 재빌드(output_dim 2~46 커버)하니 Reuters 예측이 0.73~0.75로 정상화(다른 도메인 무영향).

2. **학습 중 찍기 수준(~0.5/이진, ~0.0/다클래스) 붕괴 — 부분 완화.** 넓은 bidirectional RNN + lr=1e-2에서 그래디언트가 폭주해 네트워크가 죽는 케이스. 재현: 50k IMDB의 `1x bigru(256)`를 5개 시드로 돌리면 40%가 정확히 0.0으로 붕괴. `candidate_trainer`의 옵티마이저에 **clipnorm=1.0**을 넣으니 그 케이스는 전부 정상화. **단 완전 해결은 아님** — 재릴리스 실측에서 `3x`/`4x bigru(128)` 같은 더 깊은 스택은 clipnorm=1.0으로도 여전히 붕괴(얕은 1x/2x는 정상). release()가 실측으로 붕괴 후보를 걸러내므로 릴리스 자체는 안전.

**릴리스 영향**: 버그는 고쳤지만 재릴리스(Reuters 0.796, IMDB 0.861)가 기존(0.818/0.867)을 못 넘어 **채택하지 않음**. v0.6(bidirectional)과 같은 교훈 — "정합성·안정성 개선"이 곧 "정확도 향상"은 아니다. 스코어러/코퍼스/코드 수정은 그 자체로 옳은 개선이라 반영.

## 실험 기록 — v1.0 (검색공간 제약: 나쁜 후보를 원천 차단)

v0.9에서 clipnorm은 얕은 재귀 스택만 살렸고, 깊은 bigru + 고lr은 여전히 붕괴했다. 또 이미지 residual은 유닛이 넓으면(256) 후보당 50분+씩 잡아먹으면서 정작 승자가 된 적이 없다. 두 문제 모두 "학습을 고친다"기보다 **그런 조합을 애초에 검색공간에서 빼는 게** 실용적이라 판단, `search_space.is_valid_spec`에 두 규칙을 넣었다:

1. **재귀 인코더(lstm/gru/bilstm/bigru) 3블록 이상 + lr=1e-2 → 제외** (붕괴 상습 조합).
2. **image residual + units=256 → 제외** (초고비용, 무승).

이건 축을 없앤 게 아니라 조합만 막은 것이라 one-hot 인코딩 차원(42)은 그대로 — 스코어러 구조는 호환되고 코퍼스/스코어러만 재빌드하면 된다. 열거 크기: image 4608→4032, text 41472→36864, timeseries 12288(13824에서), tabular 2304(불변). `sample_random_spec`은 rejection sampling으로 열거기와 정확히 같은 공간에서 뽑는다.

**효과는 정확도가 아니라 엔진 효율·안정성**: 서치·릴리스 양쪽에서 붕괴/초고비용 후보를 아예 안 건드린다. 부수적으로 제약된 스코어러가 tabular에 `3x Dense(256)`를 추천해 **Tabular 0.922→0.931**로 올랐다(채택). 반면 TimeSeries 재릴리스는 0.864로 **후퇴**해 1.2(0.880) 유지 — 스코어러 재학습이 도메인마다 오르락내리락하는 "드리프트"는 v1.0에서도 여전하며, release()의 실측 검증이 그걸 걸러주는 안전장치임을 재확인.
